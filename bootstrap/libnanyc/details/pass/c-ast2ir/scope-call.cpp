...
bool Scope::visitASTExprCall(AST::Node* node, uint32_t& localvar, AST::Node* parent) {
	...
	if (not shortcircuit)
		...
		uint32_t ret__bool = ir::emit::alloc(irout, nextvar());
		// using another intermediate value to mutate it into a real object
		// if necessary
		uint32_t callret = ir::emit::alloc(irout, nextvar());
		localvar = callret; // the new expression value
		...
		emitDebugpos(*node);
		ir::emit::call(irout, ret__bool, func);
		...
		// Shortcircuit label
		uint32_t sclabel = ir::emit::label(irout, nextvar());
		...
		ir::emit::scopeEnd(irout);
		ir::emit::alloc(irout, nextvar(), nyt_u64); // allocating for sizeof
