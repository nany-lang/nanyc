...
namespace // anonymous
{
	...
	*/
	static inline std::unique_ptr<nyproject_t> createProject(const nyrun_cf_t* const runcf)
	{
		nyproject_cf_t cf;
		if (runcf)
		{
	...
	static inline std::unique_ptr<nyprogram_t> build(const nyrun_cf_t* const runcf, nyproject_t* project)
	{
		if (project)
		{
			nybuild_cf_t cf;
			...
			{
				verbose = false;
				nany_build_cf_init(&cf, project);
			}

			auto build = std::unique_ptr<nybuild_t>{nany_build_prepare(project, &cf)};
	...
	*/
	...
	std::unique_ptr<nyprogram_t> compileSource(const nyrun_cf_t* cf, const AnyString& source)
	{
		...
		if (!!project)
		{
			auto r = nany_project_add_source_n(project.get(), source.c_str(), source.size());
			if (r != nyfalse)
				return build(cf, project.get());
		}
		return nullptr;
	...
	int run(nyprogram_t* const program, const char* argv0, uint32_t argc, const char** argv)
	{
		...
		if (argc == 0 or argv == nullptr)
		{
			...
			exitstatus = nany_program_main(program, 1u, nargv);
		}
		else
		{
			...
			if (nargv)
			{
				...
				exitstatus = nany_program_main(program, argc, nargv);
				free(nargv);
			}
		}
		return exitstatus;
...
extern "C" int nany_run_n(const nyrun_cf_t* cf, const char* source, size_t length, uint32_t argc, const char** argv)
{
...
extern "C" int nany_run(const nyrun_cf_t* cf, const char* source, uint32_t argc, const char** argv)
{
...
extern "C" int nany_run_file_n(const nyrun_cf_t* cf, const char* file, size_t length, uint32_t argc, const char** argv)
{
...
extern "C" int nany_run_file(const nyrun_cf_t* cf, const char* file, uint32_t argc, const char** argv)
{
...
extern "C" void nany_run_cf_init(nyrun_cf_t* cf)
{
	if (cf)
	{
...
extern "C" void nany_run_cf_release(const nyrun_cf_t* cf)
{
	if (cf)
	{
