...
static nyio_type_t nanyc_io_fallback_statex(nyio_adapter_t*, const char*, uint32_t, uint64_t* size, int64_t* modified)
{
	assert(modified);
	assert(size);
	*modified = 0;
...
static nyio_err_t
nanyc_io_fallback_file_get_contents(nyio_adapter_t*, char** content, uint64_t* size,
