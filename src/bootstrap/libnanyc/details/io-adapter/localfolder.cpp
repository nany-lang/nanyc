...
namespace // anonymous
{
	...
	static inline nyallocator_t& retrieveAllocator(nyio_adapter_t* adapter)
	{
...
static nyio_type_t nanyc_io_localfolder_stat(nyio_adapter_t* adapter, const char* path, uint32_t len)
{
	...
	{
		case IO::typeFile:   return nyiot_file;
		case IO::typeFolder: return nyiot_folder;
		default: return nyiot_failed;
	}
}
...
namespace // anonymous
{
	...
	static inline nyio_err_t get_contents_by_small_chunks(IO::File::Stream& f, nyio_adapter_t* adapter,
...
static nyio_err_t
nanyc_io_localfolder_folder_create(nyio_adapter_t* adapter, const char* path, uint32_t len)
{
	assert(adapter != nullptr);
	auto& localpath = toLocalPath(adapter, path, len);
...
static nyio_err_t
nanyc_io_localfolder_folder_erase(nyio_adapter_t* adapter, const char* path, uint32_t len)
{
	assert(adapter != nullptr);
	auto& localpath = toLocalPath(adapter, path, len);
...
static nyio_err_t
nanyc_io_localfolder_folder_exists(nyio_adapter_t* adapter, const char* path, uint32_t len)
{
	assert(adapter != nullptr);
	auto& localpath = toLocalPath(adapter, path, len);
...
static nyio_err_t
nanyc_io_localfolder_folder_clear(nyio_adapter_t* adapter, const char* path, uint32_t len)
{
	assert(adapter != nullptr);
	auto& localpath = toLocalPath(adapter, path, len);
	bool success = false;
...
static nyio_iterator_t* nanyc_io_localfolder_folder_iterate(nyio_adapter_t* adapter, const char* path, uint32_t len,
